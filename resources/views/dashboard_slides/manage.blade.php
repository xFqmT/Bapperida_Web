<x-layouts.app :title="__('Kelola Slide Dashboard')">
        ...
        <div class="dashboard-content relative z-10 min-h-screen py-8">
            <div class="w-full px-4 sm:px-6 lg:px-8">
                <div class="max-w-7xl mx-auto space-y-6">
                    <div class="mb-6">
                        <a href="{{ route('dashboard') }}" class="inline-flex items-center text-zinc-800 dark:text-white/90 hover:text-zinc-900 dark:hover:text-zinc-100 text-sm font-medium mb-4 bg-white/90 dark:bg-zinc-800/90 px-3 py-1.5 rounded-md shadow-md backdrop-blur-sm transition-all cursor-pointer transform hover:scale-105">
                    ...
                    <div class="bg-white/80 dark:bg-zinc-800/80 backdrop-blur-sm shadow-lg rounded-xl overflow-hidden border border-zinc-200/50 dark:border-zinc-700/50">
                        <div class="px-6 py-5 border-b border-zinc-200/50 dark:border-zinc-700/50 bg-gray-50/80 dark:bg-zinc-900/80 backdrop-blur-sm flex items-center justify-between">
                            <div>
                                <h2 class="text-2xl font-bold text-gray-800 dark:text-white">Kelola Slide Dashboard</h2>
                                <p class="text-gray-500 dark:text-gray-400 mt-1 text-sm">Atur urutan, caption, dan status aktif slide dashboard</p>
                            </div>
                        ...
                        <div class="p-6 space-y-6">
                                                    ...
                                                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-colors">
                                                        ...
                                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-right">
                                                            <div class="flex items-center justify-end gap-1.5">
                                                                ...
                                                                <form action="{{ route('dashboard_slides.toggle', $slide) }}" method="POST" class="inline-block">@csrf @method('PUT')
                                                                ...
                                                                <form action="{{ route('dashboard_slides.destroy', $slide) }}" method="POST" class="inline-block" onsubmit="return confirm('Sembunyikan slide ini?')">@csrf @method('DELETE')
                            ...
                            <div id="deletedSlides" class="hidden">
                                            ...
                                            <tbody class="bg-white/50 dark:bg-zinc-800/50 divide-y divide-zinc-200/50 dark:divide-zinc-700/50 opacity-75">
                                                @foreach($deletedSlides as $slide)
                                                    <tr>
                                                        ...
                                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-right">
                                                            <form action="{{ route('dashboard_slides.restore', $slide->id) }}" method="POST" class="inline-block">@csrf
                                ...
                                @else
                                    <p class="text-center text-zinc-500 dark:text-zinc-400 py-8">Tidak ada slide yang disebunyikan</p>
                                @endif
                            </div>
                        </div>
                    </div>
            ...
            <div class="bg-white dark:bg-zinc-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    <div class="mt-3 text-center sm:mt-0 sm:text-left w-full">
                        ...
                        <form action="{{ route('dashboard_slides.store') }}" method="POST" enctype="multipart/form-data" class="mt-4 space-y-4">@csrf
                            <div>
                            ...
                            <div class="pt-2">
                                <button class="inline-flex items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 px-5 py-2.5 text-sm font-semibold text-white shadow-lg ring-1 ring-blue-600/20 transition-all cursor-pointer active:scale-95">Simpan</button>
                            </div>
                        </form>
                    </div>
                </div>
        ...
        document.addEventListener('DOMContentLoaded', function(){
            ...
            if(form){
                form.addEventListener('submit', function(){
                    ...
                    form.action = `{{ url('/dashboard-slides') }}/${id}`;
                });
            }
